...
function Get-Sequence
{
    ...
    $FilteredEvents = ($events | where message -match "Performing action on resource" )

    $SequenceArray = @()
    $string1 = 'Performing action on resource '
    $string2 = 'with sequence Id'
    ...
    $FilteredEvents | % {
        ...
        $resource = $resource.Substring(0,$resource.IndexOf($string2))

        $sequence = $_.Message.SubString($_.Message.IndexOf($string2)+$string2.Length).Trim()

        $SequenceArray += New-Object PSObject -Property @{Resource=$resource;Sequence=$sequence}
    }
